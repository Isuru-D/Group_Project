<html>
<head>
<title>Admin Dash board</title>
<link rel="stylesheet" type="text/css" href="./css/adminHome.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
<style>
	
    body {
        background-color: #000;
  		background-image: url("img/dash.jpg");
  		background-size: cover;
  		background-position: top center;
	  }
	
</style>

</head>
<body>
    <div class="container">
		<div class="container">
			<div class="prof">
				<img src="./img/profile pic.png" height="100%" width="75%">
			</div>
			<div class="companyName">
				<h1> Hello!<br /> MR. ########</h1>
			</div>
		<div class="last">
			<a href="#">Logout</a>
		</div>
	</div>
	<div class="container">
		<div class= "op1">
			<h3>MANAGE USERS</h3>
			<ul>
  				<li><a href="#home">Manage Employee<br /><i class="fa fa-user-plus" aria-hidden="true"></i></a></li>
  				<li><a href="#news">Manage Dealers<br /><i class="fa fa-users" aria-hidden="true"></i></a></li>
  			</ul>
		</div>
		<div class= "op2">
			<h3>PENDING DEALERS</h3>
			<img src="./img/pending user.png" height="70%" width="80%">
		</div>
		<div class= "op3">
			<h3>REPORT GENERATION</h3>
			<img src="./img/report.jpg" height="70%" width="80%">
		</div>
	</div>
	</div class="container">
		<div class="logo">
			<img src="img/logo.png" height =100px width=320px>
		</div>
		<div class="footer">
			<h3>copyright &copy; BrandlinkOverseas.lk</h3>
			<a href="https://www.facebook.com"><img src="img/fb.png" width=40px height=40px></a>
			<a href="https://www.instagram.com/?hl=en"><img src="img/insta.png" width=60px height=40px></a>
			<a href="https://twitter.com/"><img src="img/twitter.png" width=40px height=40px></a>
		</div>
	</div>
	</div>
</body>
</html>