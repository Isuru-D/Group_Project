...
<?php
		if(isset($_POST['add'])) {
			...
			$sql = "INSERT INTO `spareparts_stock` (`itemNO`, `name`, `quantity`, `unitPrice`, `photo`, `category`) VALUES ('$item', '$name', '$quantity', '$price', '".$img."', 'aquaflex')";
...
if(isset($_POST['search']))
{
    ...
    // mysql search query
    $query = "SELECT * FROM `spareparts_stock` WHERE `itemNo` = $id && `category`='aquaflex' LIMIT 1";
...
if(isset($_POST['update'])){
    ...
    if($photoTmpPath != ""){
        ...
        $sqly = "UPDATE spareparts_stock SET name = '".$_POST['itemName']."',itemNo = '".$_POST['itemid']."',quantity = '".$_POST['quantity']."',unitPrice = '".$_POST['price']."', photo='$image' WHERE itemNo ='".$_POST['itemid']."' && category='aquaflex'";
        $resulty = mysqli_query($connection,$sqly);
    } else{
        $sqly = "UPDATE spareparts_stock SET name = '".$_POST['itemName']."',itemNo = '".$_POST['itemid']."',quantity = '".$_POST['quantity']."',unitPrice = '".$_POST['price']."' WHERE itemNo ='".$_POST['itemid']."' && category='aquaflex'";
        $resulty = mysqli_query($connection,$sqly);   
    }
  
    $sqlz = "SELECT * FROM spareparts_stock WHERE itemNo =".$_POST['itemid']." && category ='aquaflex'";
...
if(isset($_POST['delete'])){
    ...
    $id = $_POST['itemid'];
    $sqly2 = "DELETE FROM `spareparts_stock` WHERE `itemNo` = $id && `category`='aquaflex'";
  			...
  			<div class="faux-table" aria="hidden"></div>
  				<div class="table-wrap">
    				<table class="main-table">
							...
							<?php
								$sql = "SELECT * FROM spareparts_stock WHERE category='aquaflex'";
