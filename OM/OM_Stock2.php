...
<?php
		if(isset($_POST['add'])) {
			...
			$sql = "INSERT INTO `stock` (`itemNO`, `name`, `quantity`, `unitPrice`, `photo`, `category`) VALUES ('$item', '$name', '$quantity', '$price', '".$img."', 'ferroli')";
...
if(isset($_POST['search']))
{
    ...
    // mysql search query
    $query = "SELECT * FROM `stock` WHERE `itemNo` = $id && `category`='ferroli' LIMIT 1";
...
if(isset($_POST['update'])){
    ...
    if($photoTmpPath != ""){
        ...
        $sqly = "UPDATE stock SET name = '".$_POST['itemName']."',itemNo = '".$_POST['itemid']."',quantity = '".$_POST['quantity']."',unitPrice = '".$_POST['price']."', photo='$image' WHERE itemNo ='".$_POST['itemid']."' && category='ferroli'";
        $resulty = mysqli_query($connection,$sqly);
    } else{
        $sqly = "UPDATE stock SET name = '".$_POST['itemName']."',itemNo = '".$_POST['itemid']."',quantity = '".$_POST['quantity']."',unitPrice = '".$_POST['price']."' WHERE itemNo ='".$_POST['itemid']."' && category='ferroli'";
        $resulty = mysqli_query($connection,$sqly);   
    }
  
    $sqlz = "SELECT * FROM stock WHERE itemNo =".$_POST['itemid']." && category ='ferroli'";
...
if(isset($_POST['delete'])){
  $id = $_POST['itemid'];
  $sqly2 = "DELETE FROM `stock` WHERE `itemNo` = $id && `category`='ferroli'";
  				...
  				<div class="table-wrap">
    				<table class="main-table">
        					...
        					<tr>
								...
								<?php
								$sql = "SELECT * FROM stock WHERE category='ferroli'";
