...
<?php
		if(isset($_POST['add'])) {
			...
			$sql = "INSERT INTO `stock` (`itemNO`, `name`, `quantity`, `unitPrice`, `photo`,`category`) VALUES ('$item', '$name', '$quantity', '$price', '".$img."','momali')";
...
if(isset($_POST['search']))
{
    ...
    // mysql search query
    $query = "SELECT * FROM `stock` WHERE `itemNo` = $id && `category`='momali' LIMIT 1";
	...
	if(mysqli_num_rows($result6) > 0)
    {
     ...
     //show a message and clear inputs
		...
		echo "<script> alert('Undefined ID') </script>";
			$id = "";
            $name = "";
            $quantity = "";
			$price = "";
...
// in the first time inputs are empty
	...
	$query1 = "SELECT * FROM stock WHERE category='momali'";
...
if(isset($_POST['update'])){
    ...
    if($photoTmpPath != ""){
        ...
        $sqly = "UPDATE stock SET name = '".$_POST['itemName']."',itemNo = '".$_POST['itemid']."',quantity = '".$_POST['quantity']."',unitPrice = '".$_POST['price']."', photo='$image' WHERE itemNo ='".$_POST['itemid']."' && category='momali'";
        $resulty = mysqli_query($connection,$sqly);
    } else{
        $sqly = "UPDATE stock SET name = '".$_POST['itemName']."',itemNo = '".$_POST['itemid']."',quantity = '".$_POST['quantity']."',unitPrice = '".$_POST['price']."' WHERE itemNo ='".$_POST['itemid']."' && category='momali'";
        $resulty = mysqli_query($connection,$sqly);   
    }
  
    $sqlz = "SELECT * FROM stock WHERE itemNo =".$_POST['itemid']." && category ='momali'";
...
if(isset($_POST['delete'])){
    ...
    $id = $_POST['itemid'];
    $sqly2 = "DELETE FROM `stock` WHERE `itemNo` = $id && `category`='momali'";
    				...
    				<table class="main-table">
        					...
        					<tr>
								...
								<?php
									...
									$sql = "SELECT * FROM stock WHERE category='momali'";
